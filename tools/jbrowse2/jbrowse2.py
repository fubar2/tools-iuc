    ...
    def process_annotations(self, track):
        ...
        for i, (
            ...
            if dataset_ext in ("gff", "gff3"):
                ...
                hic_path = os.path.join(self.outdir, '%s_%d_%s.hic' % (track_human_label, i, dataset_ext))
                self.subprocess_check_call(["hictk", "convert", "-f", "--output-fmt", "hic", "--tmpdir", hictempd, dataset_path, hic_path])
                self.add_hic(
                    hic_path,
                    outputTrackConfig,
                )
